...
Page({
  ...
  onName(e){this.setData({name:e.detail.value})}, onPhone(e){this.setData({phone:e.detail.value})}, onWechat(e){this.setData({wechat:e.detail.value})}, onContent(e){this.setData({content:e.detail.value})}, onType(e){this.setData({typeIndex:Number(e.detail.value)}); this.syncType()},
  submit(){
    if(!this.data.name||!this.data.phone){wx.showToast({title:this.data.t.submitNeed,icon:'none'});return}
    if(!/^1\d{10}$/.test(this.data.phone)){wx.showToast({title:this.data.t.phoneInvalid||'手机号格式错误',icon:'none'});return}
